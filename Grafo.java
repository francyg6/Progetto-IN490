...
public class Grafo {
	...
	*/
	...
	public void controllaPointers(Nodo n) {
		Nodo m=null;
		if(n.getInPointers()!=null ) {
			for(Nodo t:n.getInPointers()) {
				if(t.testLoop(n)) {
						t.removePointers(n);
						this.cfc.collapsePath(n, t);
					}
			}
		...
		ListIterator<Nodo> itr = Temp.listIterator();
			while(itr.hasNext()){
				m=itr.next();
				if (this.cfc.threadPrecedes(n,m) && !(n.testLoop(m)) && n!=m) {
					n.removePointers(m);
					this.moveSubTree(n,m);
				}else { 
					if(n.testLoop(m)) {
						n.removePointers(m);
						this.cfc.collapsePath(m, n);
	...
	private void processBacklink(Nodo v, Nodo w) {
		...
		if(v.testLoop(w)) {
			this.cfc.collapsePath(w,v);	
		}
		else if(!(w.testLoop(v))) {
			
				if(this.cfc.threadPrecedes(w,v) ) {
